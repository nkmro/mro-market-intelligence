...
// withIdempotency()와 lib/auth.js의 authenticateSession()이 실제로 호출하는 메서드
// (collection().doc().get()/set()/update()/delete(), runTransaction())만 구현한다.
// 실제 Firestore/GCP에 대한 네트워크 호출은 전혀 하지 않으며, 실제 프로덕션 코드
// (lib/writeIdempotency.js, lib/auth.js)를 그대로 이 스텁에 통과시켜 테스트한다.
// (tests/markthreadseen-parity/fake_firestore.js와 완전히 동일 — postComment/login parity
// 테스트도 같은 두 모듈(과 loginLocks 컬렉션에 대한 동일한 collection().doc() 패턴)을
// 재사용하므로 스텁 요구사항이 동일하다. 2026-08-21, logintest-parity에도 그대로 복사.)
class FakeDocRef {
