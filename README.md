...
| 시황 피드 조회·알림·실시간 배지 | `getFeed`, `getNotifications`, `getPostById`, `pollSignal` | 📋 이전 후보로 확정, 승인 대기 (`FEED_NOTIFICATIONS_CLOUDRUN_ANALYSIS.md` 참고) | `handleGetFeed_`, `handleGetNotifications_`, `handleGetPostById_`, `handlePollSignal_` | 4개 모두 순수 읽기 — 쓰기 권한 문제 없음(postComment와 다름). 4개가 공용 판정 로직(`buildFeedEntry_`)을 같이 써서, 사용자당 30초마다 불리는 `pollSignal`부터 1차 이전 후보로 검토. 실제 이전은 역할별 판정 결과 100% 동일성 검증 계획 제시 후 별도 승인 필요 |
